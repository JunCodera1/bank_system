...
- **Quản trị viên / Giao dịch viên (Admin / Teller)**: Làm việc qua bảng `Tellers` để định danh bảo mật vào mọi giao dịch trên hệ thống nội bộ.
- **Hệ thống tự động (Database Triggers)**: Đóng vai trò giám sát ngầm, tự động tính toán số dư dựa trên `status` giao dịch (`COMPLETED`), và ghi nhận Audit Log.
...
- **Đăng ký Tài khoản (Mở thẻ gốc)**:
  ...
  - Xóa mềm (Soft Delete): Tuân thủ nguyên tắc không xóa cứng KH, thiết lập `status` ACTIVE/INACTIVE/LOCKED. Bỏ `ON DELETE CASCADE` tự động.
  - Số dư khởi tạo luôn luôn phải bằng `0 VNĐ`.
  - Từ chối đăng ký nếu Email hoặc Số điện thoại đã được sử dụng trước đó.
...
- Ghi lại vết mọi giao dịch (DEPOSIT, WITHDRAWAL, TRANSFER) với thông tin chiều gửi/chiều nhận cụ thể, mã tham chiếu `reference_code` duy nhất, trạng thái định danh nhân viên `teller_id`, timestamps, trạng thái `status`, cùng lý do giao dịch.
- **Trigger Track**: Bất kỳ khi nào thông số `balance` thay đổi, phải tự động ghi vào bảng `Audit_Logs` với định dạng JSON log rõ ràng `old_data`, `new_data`.
